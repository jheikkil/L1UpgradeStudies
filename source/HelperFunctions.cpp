...
int BestIndexInRange(vector<GenericObject> &List, FourVector &Reference, double MinEta, double MaxEta, double MinPT, double DRMax, int Qual, int Type)
{
   GenericObject Best;
   int BestIndex = -1;
   ...
   for(int i = 0; i < (int)List.size(); i++)
   {
      if(List[i].P.GetAbsEta() > MaxEta || (MinEta >= 0 && List[i].P.GetAbsEta() < MinEta))
         continue;
...
GenericObject BestInRange(vector<GenericObject> &List, FourVector &Reference, double MinEta, double MaxEta, double MinPT, double DRMax, int Qual, int Type, double IsoBB, double IsoEC)
{
   ...
   for(int i = 0; i < (int)List.size(); i++)
   {
      ...
      if(Type >= 0 && List[i].Type != Type)
         continue;
      if(IsoBB >= 0 && List[i].P.GetAbsEta() < 1.5 && List[i].Iso > IsoBB)
         continue;
      if(IsoEC >= 0 && List[i].P.GetAbsEta() > 1.5 && List[i].Iso > IsoEC)
         continue;
